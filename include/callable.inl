...
struct generic_member_function
{
  ...
  static constexpr type0 check(type0) SFINAE_CHECK static constexpr type1
    check(type1) SFINAE_CHECK static constexpr type2 check(type2) SFINAE_CHECK
    static constexpr type3 check(type3) SFINAE_CHECK static constexpr type4
    check(type4) SFINAE_CHECK static constexpr type5 check(type5) SFINAE_CHECK
    static constexpr type6 check(type6) SFINAE_CHECK static constexpr type7
    check(type7) SFINAE_CHECK static constexpr type8 check(type8) SFINAE_CHECK
    static constexpr type9 check(type9) SFINAE_CHECK static constexpr type10
    check(type10) SFINAE_CHECK static constexpr type11
    check(type11) SFINAE_CHECK static constexpr type12
    check(type12) SFINAE_CHECK
    // noexcept variants
    static constexpr type13 check(type13) SFINAE_CHECK static constexpr type14
    check(type14) SFINAE_CHECK static constexpr type15
    check(type15) SFINAE_CHECK static constexpr type16
    check(type16) SFINAE_CHECK static constexpr type17
    check(type17) SFINAE_CHECK static constexpr type18
    check(type18) SFINAE_CHECK static constexpr type19
    check(type19) SFINAE_CHECK static constexpr type20
    check(type20) SFINAE_CHECK static constexpr type21
    check(type21) SFINAE_CHECK static constexpr type22
    check(type22) SFINAE_CHECK static constexpr type23
    check(type23) SFINAE_CHECK
...
decltype(std::is_pointer_v<T> &&
         std::is_function_v<std::remove_pointer_t<T>>) is_function_pointer;
  ...
  : m_empty(false)
{
  ...
  using member_function_ptr_t = decltype(
    sfinae::generic_member_function<class_type, ReturnT, ArgTs...>::check(
      member));
  using concrete_type =
    member_function<ClassT, member_function_ptr_t, ReturnT, ArgTs...>;
  ...
  m_caller = [](bool is_const, auto base, auto... arguments) {
    if (is_const) {
      auto concrete = static_cast<const concrete_type*>(base);
...
{
  using class_type = std::remove_reference_t<ClassT>;
  using call_operator_ptr_t = decltype(
    sfinae::generic_member_function<class_type, ReturnT, ArgTs...>::check(
      &class_type::operator()));
  using concrete_type =
  ...
  : m_empty(false)
{
  ...
  using member_function_ptr_t = decltype(
    sfinae::generic_member_function<class_type, ReturnT, ArgTs...>::check(
      member));
  using concrete_type = member_function_raw_pointer<ClassT,
                                                    member_function_ptr_t,
                                                    ReturnT,
                                                    ArgTs...>;
  static_assert(sizeof(concrete_type) <= Capacity, CALLABLE_ERROR);
...
{
  using member_function_ptr_t = decltype(
    sfinae::generic_member_function<ClassT, ReturnT, ArgTs...>::check(member));
  ...
  m_caller = [](bool is_const, auto base, auto... arguments) {
    if (is_const) {
      ...
      auto source_object = concrete->m_object;
      auto source_member = concrete->m_member;
      return (source_object.get()->*source_member)(
  ...
  : m_empty(false)
{
  using call_operator_ptr_t =
    decltype(sfinae::generic_member_function<ClassT, ReturnT, ArgTs...>::check(
      &ClassT::operator()));
  using concrete_type = member_function_smart_pointer<ClassT,
                                                      call_operator_ptr_t,
                                                      ReturnT,
...
template<typename ReturnT, typename... ArgTs, size_t Capacity>
callable<ReturnT(ArgTs...), Capacity>::callable(this_type& other)
{
  destroy();
  if (other.m_empty) {
  } else {
...
callable<ReturnT(ArgTs...), Capacity>::operator=(this_type& rhs)
{
  destroy();
  if (rhs.m_empty) {
    return *this;
...
{
  return (*m_caller)(false, access(), std::forward<ArgTs>(arguments)...);
}
...
{
  return (*m_caller)(true, access(), std::forward<ArgTs>(arguments)...);
}
