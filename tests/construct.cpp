...
main()
{
  // a callable is decribed by an 'unqualified' function signature and a fixed capacity specified in bytes
  // - function signature (not the return type) must not be a pointer or reference type and must not be [const, volatile, &, &&] qualified
  // - capacity[optional] is the size of allocated (stack based) storage; default value is `sizeof(std::uintptr_t) * 2`, 
  //   which is a common size for anything other than lambdas/functors that capture or contain large objects
